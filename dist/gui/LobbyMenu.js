...
function () { }
{
    ...
    $(LobbyMenu.CSS_ID.CREATE_LOBBY_FORM_SUBMIT).click(function (e) { }, {
        $(LobbyMenu) { }, : .CSS_ID.CREATE_LOBBY_FORM_SUBMIT
    }).unbind();
...
show(callback);
{
    $('.slide').fadeOut('normal', function () { }, {
        $() { }, : .empty(),
        $() { }, : .append(this.view),
        $(LobbyMenu) { }, : .CSS_ID.NICKNAME_PICK_UP
    }).modal('show');
...
updateLobbyListUI(lobby, Lobby);
{
    ...
    for (var gameLobby in gameLobbies) {
        ...
        $(LobbyMenu.CSS_ID.LOBBY_TABLE).append(' <tr><td>' + gameLobbies[gameLobby].name + '</td> ' +
