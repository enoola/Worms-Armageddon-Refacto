...
(function (Client) {
    ...
    function connectionToServer(ip, port) {
        try {
            ...
            Client.socket.on(Events.client.ASSIGN_USER_ID, function (id) { }, {
                Logger, : .debug(" Your have been assigned an id " + Client.id),
                Client, : .id = Client.id
            });
            Client.socket.on('disconnect', function () {
                Notify.display("Bad News :(", "So it looks like the game server has crashed or maybe your internet connection has been cut? " +
                    "Either way this game is over, so refresh this page now. The server will have rebooted by the time you read this hopefully.", -1, Notify.levels.error, true);
    ...
    function sendImmediately(event, packet, rateLimiter = 0) {
