...
class SettingsMenu
{
    ...
    addMapItem(map : any, name)
    {
        var item: String = '<li class="span4" style="width:30%"><a href="#" class="thumbnail" id={1}>' +
                           '<img style="width: 160px; height: 80px;" src={name}> </a></li>';
        const innerhtml: string = '<li class="span4" style="width:30%"><a href="#" class="thumbnail" id={1}>' +
            '<img style="width: 160px; height: 80px;" src={0}> </a></li>';

        string item = `innerhtml, name}`;
        formatString(innerhtml, AssetManager.getImage(map.smallImage).src);
        return item;
    }
